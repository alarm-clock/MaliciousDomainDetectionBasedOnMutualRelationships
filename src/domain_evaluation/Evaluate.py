"""
File: evaluation_api_worker.py
Author: Jozef Michal Bukas <xbukas00@stud.fit.vutbr.sk>
Date: 13.02.2026
Brief: File that contains evaluation workflow helpers for metapath2vec-based domain
    classification, including graph extraction, temporary insertion, GPU waiting,
    job state updates, CSV export, and parallel testing utilities
"""
...
def _get_graph(job: EvaluationJob, repository: GraphRepository, tmp_node_id: int) -> dgl.DGLHeteroGraph | None:
    ...
    :param tmp_node_id: `int` temporary node id of evaluated domain
...
def _check_domain_in_graph(job: EvaluationJob, repository: GraphRepository) -> None:
    """
    Method that checks whether the evaluated domain already exists in graph and, if so, finishes job immediately
    :param job: `EvaluationJob` job being processed
    :param repository: `GraphRepository` repository used for lookup
    :return: `None`
    """
...
def _check_for_duplicity(eval_job: EvaluationJob, g: dgl.DGLHeteroGraph, start_t: float) -> None:
    """
    Method that checks whether classification graph has enough label diversity for model training
    :param eval_job: `EvaluationJob` job being updated
    :param g: `dgl.DGLHeteroGraph` graph used for classification
    :param start_t: `float` overall job start time
    :return: `None`
    """
...
def parse_evaluation_result(eval_result: EvaluationJob, csv_writer) -> None:
    """
    Method that writes evaluation job result into CSV writer as one row
    :param eval_result: `EvaluationJob` evaluation job containing classification result
    :param csv_writer: CSV writer used to append a row
    :return: `None`
    """
...
def write_csv_header(csv_writer) -> None:
    """
    Method that writes CSV header for evaluation export output
    :param csv_writer: CSV writer used to append header row
    :return: `None`
    """
    csv_writer.writerow(
...
def _gen_job_from_domain_data(domain: dict[str, Any]) -> EvaluationJob:
    """
    Method that creates evaluation job object from raw domain dictionary
    :param domain: `dict[str, Any]` raw domain data
    :return: `EvaluationJob` created job object
    """
...
def parallel_test(provider, class_out_f_name: str) -> None:
    """
    Method that evaluates many domains in parallel and writes results into CSV file
    :param provider: iterable providing domain dictionaries
    :param class_out_f_name: `str` output CSV file path
    :return: `None`
    """
...
def test_from_collection(path_to_config: str, class_out_f_name: str, filter_train: bool) -> None:
    """
    Method that loads MongoDB configuration, extracts domain collection, and runs parallel test export
    :param path_to_config: `str` path to JSON configuration file
    :param class_out_f_name: `str` output CSV file path
    :param filter_train: `bool` flag deciding whether training domains should be filtered out
    :return: `None`
    """
