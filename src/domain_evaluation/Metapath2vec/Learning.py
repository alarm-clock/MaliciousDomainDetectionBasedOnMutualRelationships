...
Date: 15.02.2026
Brief: File that contains logic for domain classification using MetaPath2Vec-generated
    graph embeddings and logistic regression over domain neighborhood features
"""
...
def _train_regress(model_embedding: th.Tensor, train_mask,
    ...
    """
    Method that trains logistic regression classifier on generated embeddings and returns
    classification probabilities for nodes selected by classification mask
    :param model_embedding: `th.Tensor` tensor containing node embeddings
    :param train_mask: tensor-like mask specifying which nodes are used for training
    :param classify_mask: `th.Tensor` mask specifying which nodes should be classified
    :param labels: `th.Tensor` tensor containing node labels
    :param scale: `bool` flag specifying whether embeddings should be standardized before classification
    :return: `th.Tensor` predicted class probabilities for classified nodes
    """

    # Move embeddings and labels to CPU because scikit-learn operates on CPU arrays.
    ...
    # Optionally normalize embeddings before fitting logistic regression model.
    if scale:
        sclr = StandardScaler()
...
def _train_log_regress_and_cls(
        ...
        scale: bool = True) :
    """
    Method that trains logistic regression on embeddings generated for multiple meta-paths
    and combines resulting probabilities into final classification output
    :param g: `dgl.DGLHeteroGraph` graph containing training and testing node masks and labels
    :param embeddings: `list[th.Tensor]` list of embedding tensors generated from different meta-paths
    :param path_names: `list[str]` names corresponding to generated embedding sets
    :param mode: `bool` flag specifying whether production or testing output format should be used
    :param scale: `bool` flag specifying whether embeddings should be standardized before classification
    :return: average probability vector in production mode, otherwise dictionary with detailed results
    """
    ...
    # Train one classifier for each embedding space generated from meta-path2vec.
    ...
    # Average per-model probabilities to obtain final output probability vector.
    avg_mal = sum(avg_0) / len(avg_0)
    abg_ben = sum(avg_1) / len(avg_1)
    results_dict['AVERAGE'] = numpy.array([avg_mal, abg_ben])

    # In testing mode also evaluate concatenated embedding representation.
    if mode == TESTING:
...
def _train_metapath2vec(model_loader: tuple[MetaPath2Vec, DataLoader, str], device: th.device, lr: float, cnt: int) -> tuple[th.Tensor, list[float], str]:
    """
    Method that trains one MetaPath2Vec model and returns generated embeddings together
    with loss history and path identifier
    :param model_loader: `tuple[MetaPath2Vec, DataLoader, str]` tuple containing model,
        corresponding dataloader, and path name
    :param device: `th.device` device on which model training should run
    :param lr: `float` learning rate for sparse optimizer
    :param cnt: `int` sequential identifier of trained model
    :return: `tuple[th.Tensor, list[float], str]` generated domain embeddings, collected losses,
        and path name
    """
    ...
    # Train model for fixed number of epochs over generated walk samples.
    ...
    # Extract learned embeddings for domain nodes only.
...
def _train_metapath2vec_safe(
    ...
    """
    Method that safely creates and trains one MetaPath2Vec model for given meta-path
    and returns None if model cannot be created for graph
    :param device: `th.device` device on which model training should run
    :param lr: `float` learning rate for sparse optimizer
    :param cnt: `int` sequential identifier of trained model
    :param g: `dgl.DGLHeteroGraph` graph on which embeddings are generated
    :param m_path: `list[str]` meta-path used for random walk generation
    :param w_size: `int` context window size
    :param emb_dim: `int` embedding dimension
    :param neg_size: `int` number of negative samples
    :return: `tuple[th.Tensor, list[float], str] | None` generated embeddings, losses,
        and path name on success, otherwise None
    """
    ...
    # Use fixed random seed for reproducible random walk sampling.
...
def _gen_embeds_parallel_safe(
    ...
    """
    Method that generates embeddings for multiple meta-paths in parallel while safely
    skipping invalid meta-path configurations
    :param device: `th.device` device on which model training should run
    :param g: `dgl.DGLHeteroGraph` graph on which embeddings are generated
    :param m_paths: `list[list[str]]` list of meta-paths for which embeddings should be trained
    :param w_size: `int` context window size
    :param emb_dim: `int` embedding dimension
    :param neg_size: `int` number of negative samples
    :param lr: `float` learning rate for sparse optimizer
    :return: `tuple[list[th.Tensor], list[list[float]], list[str]]` generated embeddings,
        losses, and path names for all successfully trained models
    """

    # Train one model per meta-path using multiprocessing pool.
...
def get_class_counts(g: dgl.DGLHeteroGraph):
    """
    Method that returns counts of class labels present in training subset of graph domain nodes
    :param g: `dgl.DGLHeteroGraph` graph containing labels and training mask
    :return: tuple of unique labels and their counts
    """
    labels = g.ndata['l']['d']
...
def check_for_duplicity(g: dgl.DGLHeteroGraph) -> bool | tuple:
    ...
    # Classification requires at least two domain nodes in sampled graph.
    ...
    # If all neighborhood nodes belong to one class, direct probability can be inferred.
...
def classify_domain(g: dgl.DGLHeteroGraph, eval_result: EvaluationResult, mode: bool = PRODUCTION, no_need_for_correct_check: bool = False) -> bool:
    ...
    # Optionally verify that graph structure and class distribution are sufficient for classification.
    ...
    # Load model hyperparameters from application configuration.
    ...
    emb_dim = Config.get_instance().eval_app_conf.eval_params.embedd_dim
    lr = Config.get_instance().eval_app_conf.eval_params.lr

    # Generate embeddings for selected meta-paths and classify target domain.
