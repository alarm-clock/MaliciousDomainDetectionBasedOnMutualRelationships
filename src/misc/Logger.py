"""
File: Logger.py
Author: Jozef Michal Bukas <xbukas00@stud.fit.vutbr.cz>
Date: 27.12.2025
Brief: File that contains singleton logger implementation for application logging,
    runtime statistics, and evaluation result recording
"""

import threading
from datetime import datetime
from enum import Enum
import psutil


class MyLogger:
    """
    Class that represents singleton logger used for runtime logging and statistics collection
    """

    _instance_ = None

    class LogLevel(Enum):
        """
        Enum that represents available logging levels
        """
        DEBUG = ('DEBUG', 0)
        INFO = ('INFO', 1)
        WARNING = ('WARNING', 2)
        ERROR = ('ERROR', 3)

    def __new__(cls, *args, **kwargs):
        """
        Method that ensures only one logger instance exists
        :return: `MyLogger` singleton logger instance
        """
        if cls._instance_ is None:
            cls._instance_ = super().__new__(cls)
            cls._instance_.initialized = False
        return cls._instance_

    def __init__(self, log_file: str | None = None, log_level: LogLevel = LogLevel.INFO) -> None:
        """
        Method that initializes logger instance only once
        :param log_file: `str | None` path to main log file
        :param log_level: `LogLevel` active logging level
        :return: None
        """
        if not self.initialized:
            self._lock = threading.Lock()
            self._result_lock = threading.Lock()
            self._stats_lock = threading.Lock()
            self._logfile = log_file
            self._log_level = log_level
            self._n_req = 0
            self._n_timeout = 0
            self._n_fin = 0
            self._GB = 1024**3
            self._whole_time = 0.0
            self._last_time = 0.0

            self._result_log_file = "evaluation_results.log"
            with open(self._result_log_file, 'a') as f:
                f.write(("=" * 35) + " Log start " + ("=" * 35) + '\n')
                f.write("domain_name in_graph no_neighbor class mal_prob ben_prob fin_state err_descr\n")

            if log_file is not None:
                with open(self._logfile, 'a') as f:
                    f.write(("=" * 35) + " Log start " + ("=" * 35) + '\n')

            self.initialized = True

    @staticmethod
    def get_instance(log_file: str | None = None):
        """
        Method that returns singleton logger instance
        :param log_file: `str | None` optional log file used during first initialization
        :return: `MyLogger` singleton logger instance
        """
        if MyLogger._instance_ is None:
            MyLogger(log_file)
        return MyLogger._instance_

    @staticmethod
    def _create_log_msg(msg: str, level: LogLevel) -> str:
        """
        Method that creates formatted log message with timestamp and level
        :param msg: `str` logged message
        :param level: `LogLevel` log level
        :return: `str` formatted log message
        """
        now = datetime.now()
        return now.strftime("%Y-%m-%d %H:%M:%S.%f") + f"-{level.value[0]}" + ': ' + msg + '\n'

    def add_req(self) -> None:
        """
        Method that increments total request counter
        :return: None
        """
        self._n_req += 1

    def add_timeout(self) -> None:
        """
        Method that increments timeout counter
        :return: None
        """
        self._stats_lock.acquire()
        self._n_timeout += 1
        self._stats_lock.release()

    def add_finished(self) -> None:
        """
        Method that increments finished-request counter
        :return: None
        """
        self._n_fin += 1

    def add_time_taken(self, time: float) -> None:
        """
        Method that records processing time statistics
        :param time: `float` processing time
        :return: None
        """
        self._stats_lock.acquire()
        self._last_time = time
        self._whole_time += time
        self._stats_lock.release()

    def log_stats(self) -> tuple[int, int, int, float, float, float, float]:
        """
        Method that logs and returns collected runtime statistics
        :return: `tuple[int, int, int, float, float, float, float]` statistics summary
        """

        mem = psutil.virtual_memory()
        self._stats_lock.acquire()
        avg_time = self._whole_time / self._n_fin if self._n_fin > 0 else 0
        last_time = self._last_time
        self.log(
            f"""Tasks (Total: {self._n_req}, finished: {self._n_fin}, timeout: {self._n_timeout}) 
        \t\tMem: (Used: {mem.percent}%, available: {mem.available / self._GB}GB)
        \t\tJob_time: (Avg: {avg_time}, Last: {last_time})""")
        self._stats_lock.release()
        return self._n_req, self._n_fin, self._n_timeout, mem.percent, mem.available / self._GB, avg_time, last_time

    def _concurent_log(self, msg: str, level: LogLevel = LogLevel.INFO, with_time: bool = True) -> None:
        """
        Method that writes one log message into log file in thread-safe manner
        :param msg: `str` logged message
        :param level: `LogLevel` log level
        :param with_time: `bool` flag indicating whether timestamp should be included
        :return: None
        """
        self._lock.acquire()
        with open(self._logfile, 'a') as f:
            f.write(self._create_log_msg(msg, level) if with_time else msg + '\n')
        self._lock.release()

    def log(self, msg: str):
        """
        Method that logs info-level message
        :param msg: `str` logged message
        :return: None
        """
        if self._logfile is None or self._log_level.value[1] > MyLogger.LogLevel.INFO.value[1]:
            return

        threading.Thread(target=self._concurent_log, args=(msg,)).start()

    def log_error(self, msg: str) -> None:
        """
        Method that logs error-level message
        :param msg: `str` logged message
        :return: None
        """
        if self._logfile is None:
            return

        threading.Thread(target=self._concurent_log, args=(msg, MyLogger.LogLevel.ERROR)).start()

    def log_warning(self, msg: str) -> None:
        """
        Method that logs warning-level message
        :param msg: `str` logged message
        :return: None
        """
        if self._logfile is None or self._log_level.value[1] > MyLogger.LogLevel.WARNING.value[1]:
            return

        threading.Thread(target=self._concurent_log, args=(msg, MyLogger.LogLevel.WARNING)).start()

    def log_debug(self, msg: str, concurrent_log: bool = True, with_time: bool = True) -> None:
        """
        Method that logs debug-level message
        :param msg: `str` logged message
        :param concurrent_log: `bool` flag indicating whether logging should run in separate thread
        :param with_time: `bool` flag indicating whether timestamp should be included
        :return: None
        """
        if self._logfile is None or self._log_level.value[1] > MyLogger.LogLevel.DEBUG.value[1]:
            return

        if concurrent_log:
            threading.Thread(target=self._concurent_log, args=(msg, MyLogger.LogLevel.DEBUG, with_time)).start()
        else:
            self._concurent_log(msg, MyLogger.LogLevel.DEBUG, with_time)

    def log_evaluation_result(self, job) -> None:
        """
        Method that logs one evaluation result into dedicated result log file
        :param job: evaluated job object
        :return: None
        """

        domain_name = job.domain_name
        state = job.state.value
        err_descr = job.error_description
        in_graph = job.result.in_graph
        no_neighbor = job.result.no_neighbor
        malicious = job.result.malicious
        m_p, b_p = job.result.get_probability
        msg = f"{domain_name} {in_graph} {no_neighbor} {'m' if malicious else 'b'} {m_p} {b_p} {state} {err_descr}"

        eval_time = datetime.now()
        msg = f"{eval_time.strftime('%Y-%m-%d %H:%M:%S.%f')}: {msg} \n"
        with self._result_lock:
            with open(self._result_log_file, 'a') as f:
                f.write(msg)