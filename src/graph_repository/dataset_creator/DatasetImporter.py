"""
File: DatasetImporter.py
Author: Jozef Michal Bukas <xbukas00@stud.fit.vutbr.cz>
Date: 20.02.2026
Brief: File that contains dataset importer class used for building graph structures
    from MongoDB datasets, dispatching dataset workers, creating DGL heterographs,
    and importing constructed graph data into Neo4j
"""
...
class DatasetImporter:
    ...
    def __init__(self, edge_types: str | None = None, ranges: str | None = None, no_lone_nd: bool = False,
        ...
        """
        # Store optional Neo4j configuration path for later import phase.
        ...
        # Dynamically import all dataset worker modules and collect supported edge definitions.
        ...
        # If full graph range is requested implicitly, determine maximum node id from collection.
        if max_id == 2 ** 32:
    ...
    def _store_n_data_neo4j(self, n_t: str, n_data: dict[str, list]) -> None:
        """
        Method that stores node data in Neo4j-oriented row representation
        :param n_t: `str` Neo4j node label
        ...
        if self._n_data_neo4j.get(n_t) is None:
            ...
            # Merge newly received properties into already stored rows for given node label.
    ...
    def _store_n_data(self, n_t: NodeTypes, n_data: dict[str, list]) -> None:
        """
        Method that dispatches node data storage to DGL or Neo4j representation
        :param n_t: `NodeTypes` node type
        :param n_data: `dict[str, list]` node data to store
        :return: None
        """
    ...
    def _store_edge(self, e_type: tuple[NodeTypes, EdgeTypes, NodeTypes], u: list[int], v: list[int],
                    e_data: tuple[str, list] | None = None) -> None:
        """
        Method that stores created edges and optional edge data in currently selected backend format
        :param e_type: `tuple[NodeTypes, EdgeTypes, NodeTypes]` edge type triple
        :param u: `list[int]` source node ids
        :param v: `list[int]` target node ids
        :param e_data: `tuple[str, list] | None` optional edge attribute name and values
        :return: None
        """
    ...
    def _debug_edges_by_type(self):
        """
        Method that prints debug information about collected edges and stored data
        :return: None
        """
    ...
    def _create_domain_name_indexes(driver: Neo4jDBDriver) -> bool:
        """
        Method that creates Neo4j indexes on `domain_name` for all dummy and domain labels
    ...
    def replace_other_dummies_with_default_dummy_domain(driver: Neo4jDBDriver) -> None:
        """
        Method that merges service-specific dummy domain nodes into default dummy domain label
        :param driver: `Neo4jDBDriver` Neo4j driver instance
        :return: None
        """
    ...
    def _import_into_neo4j(self):
        """
        Method that imports already prepared nodes and edges into Neo4j database
        :return: None
        """
    ...
    def _add_maintenance_values_to_nodes(self):
        """
        Method that adds maintenance properties required for Neo4j graph versioning
        :return: None
        """
    ...
    def create_graph_and_import_to_neo4j(self):
        """
        Method that creates graph from dataset and imports it into Neo4j
        :return: None
        """
