...
Date: 14.3.2026
Brief: File that contains edit-loop logic for processing prioritized graph-editing
    requests, creating new graph versions, and gracefully handling finish signals
"""
...
class FinishType(Enum):
    """
    Class that represents possible edit-loop finishing modes
    """
...
def edit_loop(stop_event: threading.Event, queue: PriorityQueue[GraphRequest], driver_conf_file: str) -> None:
    ...
    while not stop_event.is_set():
        ...
        try:
            request = queue.get(timeout=15.0)
        except Empty:
