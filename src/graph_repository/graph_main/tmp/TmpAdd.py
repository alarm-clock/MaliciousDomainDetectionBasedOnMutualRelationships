...
Date: 13.01.2026
Brief: File that contains helper functions for creating temporary domains and
    their relationships to existing graph nodes
"""
...
def _get_edges(available_options: list[TMP_FUNC_T], domain: dict[str, Any], driver: Neo4jDBDriver, version: int, tmp_node_id) -> EDGES_T | None:
    ...
    :param tmp_node_id: allocated temporary node id
...
def _prepare_domain_for_adding(domain: dict[str, Any], version: int, tmp_node_id: int) -> dict[str, Any]:
    """
    Function that prepares temporary domain node data for insertion into graph
    :param domain: `dict[str, Any]` temporary domain data
    :param version: `int` target graph version
    :param tmp_node_id: `int` allocated temporary node id
    :return: `dict[str, Any]` prepared node data
    """
...
def add_temporary_domain(domain: dict[str, Any], version: int, driver: Neo4jDBDriver) -> int | None:
    ...
    :param domain: `dict[str|Any]` that holds temporary domain data, must at least hold domain name otherwise function fails
