...
Date: 27.12.2025
Brief: File that contains main CLI entry point for dataset import, graph editing,
...
def add_signal_handlers():
    """
    Function that registers signal handlers for graceful application shutdown
    :return: None
    """
    def _graceful_exit(signum, frame):
        """
        Function that stops running application services before program exit
...
def main():
    ...
    if args.log is not None:
        ...
        log_level_str = conf.logging_conf.log_level
        no_match = True
        for level in MyLogger.LogLevel:
            if level.value[0] == log_level_str:
                no_match = False
        ...
        MyLogger(conf.logging_conf.log_file, log_level)
