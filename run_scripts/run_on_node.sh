#!/bin/bash
#PBS -N testing_on_new-domains
#PBS -q gpu
#PBS -l select=1:ncpus=2:mem=300gb:ngpus=1:scratch_local=10gb
...
RUN_SCRIPT=$PROJECT_DIR/singularity_exec.sh
...
singularity exec -B $BRNO_HOME/mongodb_files:/data/db $CONTAINER $PROJECT_DIR/run_scripts/singularity_exec.sh 
